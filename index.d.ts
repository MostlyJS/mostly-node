...
declare type AddHandler = (this: MostlyNode, request: Pattern, reply?: Reply) => void;
declare type ActHandler = (this: MostlyNode, error: Error, response: ClientResult) => void;

declare type ExtensionNextHandler = (error: Error) => void;
declare type ExtensionHandler = (ctx: MostlyNode, request: any, response: any, next?: ExtensionNextHandler) => void;

interface Plugins {
  [name: string]: PluginDefinition;
}

declare class MostlyNode extends events.EventEmitter {
  ...
  on(event: MostlyEvents, handler: Function);
  removeAll();
